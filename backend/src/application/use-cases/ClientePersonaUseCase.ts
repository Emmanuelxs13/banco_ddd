...
export class ClientePersonaUseCase {
  ...
  async create(data: {
    ...
    const existente = await this.repo.findByIdentificacion(data.numero_identificacion);
    if (existente) throw new ValidationError(`Ya existe cliente con identificación ${data.numero_identificacion}`);

    const fechaNac = new Date(data.fecha_nacimiento);
    const cliente = new ClientePersona(
      null, data.numero_identificacion, data.nombre_completo,
      data.correo_electronico, data.telefono, fechaNac,
